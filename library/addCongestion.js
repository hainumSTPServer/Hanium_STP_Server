...
const addCongestion = async realTimeArray => {
  ...
  for (let i in realTimeArray) {
    const congestionData =
      await congestionServiceInstance.getCongestionByStationId(
        month,
        hour,
        realTimeArray[i].stationId,
      );
    realTimeArray[i].congestion = Math.floor(
      congestionData.congestion * dayWeightedValue[day],
    );
  }
